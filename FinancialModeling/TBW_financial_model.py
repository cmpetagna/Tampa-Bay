...
def get_HarneyAugmentationFromOMS(mat_file_name_path = 'C:/Users/dgorelic/Desktop/TBWruns/rrv_0125' + '/sim_0001.mat', 
    ...
    ###FIX one_thousand_added_to_read_files = 1000
    ###FIX 'sim_0' + str(one_thousand_added_to_read_files + r_id)
    if r_id < 10:
        sim_prefix = 'sim_000'
    elif 10 <= r_id < 100:
        sim_prefix = 'sim_00'
    ...
    all_harney_days = oms_file[sim_prefix + str(r_id)]['HRTBC']['HAug'][:]
...
###TA
def EYI_calculation(principal, interest_rate, maturity):
    total_annual_payment = principal * ((interest_rate * (1 + interest_rate)**maturity)/((1 + interest_rate)**maturity -1))
    return total_annual_payment

###TA
def expected_payments(dataframe, EYI_payment, principal, interest_rate):
    balance = principal
    for year in range(len(dataframe.iloc[:,2])):
        if dataframe.iloc[year,2] == 0:
            dataframe.iloc[year, 1] = principal
            dataframe.iloc[year, 3] = interest_rate * principal
            dataframe.iloc[year, 4] = interest_rate * principal
        else:
            interest = balance * interest_rate
            principal_component = EYI_payment - interest
            principal_paid = min(principal_component, balance)
            balance -= principal_paid
            dataframe.iloc[year, 1] = balance
            dataframe.iloc[year, 2] = principal_paid
            dataframe.iloc[year, 3] = interest
            dataframe.iloc[year, 4] = EYI_payment
...
def bond_series_agreements(fiscal_years_to_keep, debt_service_schedule_Bond2023, debt_service_schedule_Bond2, debt_service_schedule_Bond3, debt_service_schedule_Bond4, \
    ...
    #Bring in the principal amount for each bond
    bond2023_principal = dv_list[23]
    bond2_principal = dv_list[24]
    bond3_principal = dv_list[25]
    bond4_principal = dv_list[26]
    
    for fy in fiscal_years_to_keep:
        if fy == 2023:
            debt_service_schedule_Bond2023['Principal Amount Remaining'].loc[debt_service_schedule_Bond2023['Fiscal Year'] >= fy] = bond2023_principal
        elif fy == Bond2_issue_year:
            debt_service_schedule_Bond2['Principal Amount Remaining'].loc[debt_service_schedule_Bond2['Fiscal Year'] >= fy] = bond2_principal
        elif fy == Bond3_issue_year:
            debt_service_schedule_Bond3['Principal Amount Remaining'].loc[debt_service_schedule_Bond3['Fiscal Year'] >= fy] = bond3_principal
        elif fy == Bond4_issue_year:
            debt_service_schedule_Bond4['Principal Amount Remaining'].loc[debt_service_schedule_Bond4['Fiscal Year'] >= fy] = bond4_principal
    ...
    #Find the equated yearly installment for each bond (EYI usually referred to as EMI)
    #Need to subract the amount of years that there is not principal payment from the maturity to get an accurate EMI calculation
    bond2023_EYI = EYI_calculation(bond2023_principal, interest_rate_2023bond, (maturity_Bond2023-years_with_no_principal_payment_B2023))
    bond2_EYI = EYI_calculation(bond2_principal, interest_rate_Bond2, (maturity_Bond2-years_with_no_principal_payment_B2))
    bond3_EYI = EYI_calculation(bond3_principal, interest_rate_Bond3, (maturity_Bond3 - years_with_no_principal_payment_B3))
    bond4_EYI = EYI_calculation(bond4_principal, interest_rate_Bond4, (maturity_Bond4 - years_with_no_principal_payment_B4))
    
    #Calculate budgeted total, principal, and interest payments
    debt_service_schedule_Bond2023.iloc[(2023 - (first_modeled_fy-1)):, :] = expected_payments(debt_service_schedule_Bond2023.iloc[(2023 - (first_modeled_fy-1)):, :], bond2023_EYI, bond2023_principal, interest_rate_2023bond)
    debt_service_schedule_Bond2.iloc[(Bond2_issue_year - (first_modeled_fy-1)):,:] = expected_payments(debt_service_schedule_Bond2.iloc[(Bond2_issue_year - (first_modeled_fy-1)):,:],  bond2_EYI, bond2_principal, interest_rate_Bond2)
    debt_service_schedule_Bond3.iloc[(Bond3_issue_year - (first_modeled_fy-1)):,:] = expected_payments(debt_service_schedule_Bond3.iloc[(Bond3_issue_year - (first_modeled_fy-1)):,:], bond3_EYI, bond3_principal, interest_rate_Bond3)
    debt_service_schedule_Bond4.iloc[(Bond4_issue_year - (first_modeled_fy-1)):,:] = expected_payments(debt_service_schedule_Bond4.iloc[(Bond4_issue_year - (first_modeled_fy-1)):,:], bond4_EYI, bond4_principal, interest_rate_Bond4)
...
def pull_ModeledData(additional_scripts_path, orop_output_path, oms_output_path, realization_id, 
    ...
    ###FIX one_thousand_added_to_read_files = 1000
    n_days_in_year = 365
    if realization_id < 10:
        sim_prefix = '_000'
    elif 10 <= realization_id < 100:
        sim_prefix = '_00'
    elif 100 <= realization_id < 1000:
        sim_prefix = '_0'
    else:
        sim_prefix = '_'
    ...
    if end_fiscal_year > first_modeled_fy: # meaning the last FY modeled financially is 2020
        ...
        if PRE_CLEANED:
            AMPL_cleaned_data = pd.read_csv(orop_output_path + '/ampl' + sim_prefix + str(realization_id) + '.csv')
        else:
            AMPL_cleaned_data = read_AMPL_csv(orop_output_path + '/ampl' + sim_prefix + str(realization_id) + '.csv', export = False)
        ...
        TBC_raw_sales_to_CoT = get_HarneyAugmentationFromOMS(oms_output_path + '/sim' + sim_prefix + str(realization_id) + '.mat', ndays_of_realization, realization_id)
...
#    other_transfer_factor = rdm_factor_list[12]
    ...
    if current_FY_budget_surplus < 0:
        ...
        if current_FY_budget_surplus > current_FY_needed_reserve_deposit:
            ...
            current_FY_needed_reserve_deposit -= current_FY_needed_reserve_deposit_realized
            #So then are you now short the remaining needed deposit? 3/22
        ...
        # if it was earlier depleted 3/22 (presumable depleted in previous FY - where else w)
...
#    print(str(FY) + ': Surplus Deposit in Rate Stabilization is ' + str(current_FY_rate_stabilization_fund_deposit))
    ...
    financial_metrics.loc[financial_metrics['Fiscal Year'] == FY] = \
        pd.DataFrame([FY,
                        ...
                        total_deficit,
                        current_FY_debt_service,
                        current_FY_cip_deposit]).transpose().values
...
def run_FinancialModelForSingleRealization(start_fiscal_year, end_fiscal_year,
    ...
    # give number of variables tracked in outputs
    n_financial_metric_outcomes = 19
    ...
    n_debt_service_schedule_Bond2023_variables = 5
    n_debt_service_schedule_Bond2_variables = 5
    n_debt_service_schedule_Bond3_variables = 5
    n_debt_service_schedule_Bond4_variables = 5
    ...
    ###TA
    debt_service_schedule_Bond2023, debt_service_schedule_Bond2, debt_service_schedule_Bond3, debt_service_schedule_Bond4 = bond_series_agreements(fiscal_years_to_keep, debt_service_schedule_Bond2023, debt_service_schedule_Bond2, 
...
local_MonteCarlo_data_base_path = 'D:/input_data'
...
#DVs.index = np.arange(1, len(DVs) + 1)
...
#DUFs.index = np.arange(1, len(DUFs) +1)
...
inflation_matrix = pd.read_csv(dv_path + '/inflation_scenarios.csv', header = None)
#inflation_matrix.index = np.arange(1, len(inflation_matrix) +1)
###inflation_matrix = pd.read_csv(dv_path + '/inflation_scenarios.csv', header = None) #Vgrid pathway

### ---------------------------------------------------------------------------
# read in historic records
historical_data_path = local_MonteCarlo_data_base_path + '/budget_data'
...
### =========================================================================== ###
for run_id in [162]: # NOTE: DAVID'S LOCAL CP ONLY HAS 125 RUN OUTPUT FOR TESTING
    ...
    n_reals_tested = 1000 # NOTE: DAVID'S LOCAL CP ONLY HAS RUN 125 MC REALIZATION FILES 0-200 FOR TESTING
    #These for sim is for the old version of the code where one financial scenario runs across all 1,000 realizations
    #for sim in range(0,2): # sim = 0 for testing
    #for sim in range(0,1): # FOR RUNNING HISTORICALLY ONLY
    for sim in range(21,23): # FOR RUNNING MULTIPLE SIMULATIONS 
    ###TA This matches one financial scenario with one realization. Specify scenario number to better pull data for postprocessing
    # sim = 7
    # r_id = 1
    # for r_id, (sim_dvs, sim_dufs, sim_infl) in enumerate(zip(DVs.itertuples(), DUFs.itertuples(), inflation_matrix.itertuples())):
    #     r_id += 1
        ...
        ###TA
        # dvs = list(sim_dvs[1:])
        # dufs = list(sim_dufs[1:])
        # annual_inflation = list(sim_infl[1:])
        # This is for the old version of the code where one financal scenario runs across all 1,000 realizations
        ...
        #     print(r_id)
            ...
            #as of run 162 - this is no longer an issue.
        #     if r_id == 95:
        #         continue
    ...
    #    for r_id in range(1,2): # r_id = 1 for testing
            ...
            #This works when the realizations are running throught the scenario for loop.
        ...
        VR.to_csv(output_path + '/VR_f' + str(run_id) + '_s' + str(sim) + '.csv')
        WD.to_csv(output_path + '/WD_f' + str(run_id) + '_s' + str(sim) + '.csv')
