...
realization_groupings = pd.read_csv('C:/Users/cmpet/OneDrive/Documents/UNC Chapel Hill/TBW/Analysis/demand_analysis/last_yr_demand_division.csv', index_col=0)
low_demand_realizations = list(realization_groupings.iloc[:,0])
lowmed_demand_realizations = list(realization_groupings.iloc[:,1])
med_demand_realizations = list(realization_groupings.iloc[:,2])
high_demand_realizations = list(realization_groupings.iloc[:,3])
low_demand_realizations = [x for x in low_demand_realizations if math.isnan(x) == False]
lowmed_demand_realizations = [x for x in lowmed_demand_realizations if math.isnan(x) == False]
med_demand_realizations = [x for x in med_demand_realizations if math.isnan(x) == False]
high_demand_realizations = [x for x in high_demand_realizations if math.isnan(x) == False]
low_demand_realizations = [int(x + 1) for x in low_demand_realizations]
lowmed_demand_realizations = [int(x +1) for x in lowmed_demand_realizations]
med_demand_realizations = [int(x+1) for x in med_demand_realizations]
high_demand_realizations = [int(x+1) for x in high_demand_realizations]
all_realizations = [x for x in range(1,1001)]
...
data_names_to_plot = ['Utility Reserve Fund Balance (Total)', 'R&R Fund (Total)', 'Rate Stabilization Fund (Total)', 'CIP Fund (Total)']

formulation_to_plot = [162] # list all
simulation_to_plot = [16, 19] # list all, IDs start at 0
realization_to_plot = all_realizations # list which we want, IDs start at 1 not zero
...
for f in range(0,len(formulation_to_plot)):
    ...
    for s in range(0,len(simulation_to_plot)):
        ...
        for r in realization_to_plot:
            print('\t\tCollecting output for Realization ' + str(r))
            
            # if realization_to_plot[r] == 95:
            #     continue
                
            # read data input files
            actuals = pd.read_csv(data_path + '/budget_actuals_f' + str(formulation_to_plot[f]) + '_s' + str(simulation_to_plot[s]) + '_r' + str(r) + '.csv', index_col = 0)
            budgets = pd.read_csv(data_path + '/budget_projections_f' + str(formulation_to_plot[f]) + '_s' + str(simulation_to_plot[s]) + '_r' + str(r) + '.csv', index_col = 0)
            metrics = pd.read_csv(data_path + '/financial_metrics_f' + str(formulation_to_plot[f]) + '_s' + str(simulation_to_plot[s]) + '_r' + str(r) + '.csv', index_col = 0)
